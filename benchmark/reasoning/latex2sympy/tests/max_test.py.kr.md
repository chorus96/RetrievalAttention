# `benchmark/reasoning/latex2sympy/tests/max_test.py` 분석

## 개요
latex2sympy2의 **최댓값 max 변환을 검증하는 pytest 테스트**입니다. 여러 LaTeX 입력을 `process_sympy`로 변환해 기대 SymPy 표현식과 일치하는지 확인합니다.
...
```mermaid
flowchart TD
    C["케이스 목록<br/>max 관련 LaTeX"] --> P["@parametrize"]
    P --> AE["assert_equal"]
...
- 최댓값 max 관련 문법 변환의 회귀(regression)를 막는 것이 목적입니다.
